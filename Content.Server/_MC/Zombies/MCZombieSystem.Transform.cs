...
/// </remarks>
public sealed partial class MCZombieSystem
{
    ...
    /// </summary>
    private void OnDamageChanged(EntityUid uid, MCZombifyOnDeathComponent component, MobStateChangedEvent args)
    {
    ...
    public void ZombifyEntity(EntityUid target, MobStateComponent? mobState = null)
    {
        ...
        if (HasComp<MCZombieComponent>(target) || HasComp<MCZombieImmuneComponent>(target))
            return;
        ...
        var zombiecomp = AddComp<MCZombieComponent>(target);
        ...
        if (TryComp<MCZombieAccentOverrideComponent>(target, out var accent))
        ...
        RemCompDeferred<MCPendingZombieComponent>(target);
